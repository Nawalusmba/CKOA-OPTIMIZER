...
%_________________________________________________________________________%
%%
%%%%utilisation de la carte chebychev dans les lignes suivantes : 
%%%%   ligne 36   ligne 39   ligne 67    ligne 75    
%%%%   ligne 89   ligne 93   ligne 105   ligne 128  ligne 139
% The Kepler Optimization Algorithm
function [Sun_Score,Sun_Pos,Convergence_curve]=KOA_CHAOS1(SearchAgents_no,Tmax,ub,lb,dim,fobj,fhd)
...
%**********utilisation du chao-chebyshev
 %orbital=rand(1,SearchAgents_no); %% Eq.(4)  
orbital=abs(chaos1(1,rand,SearchAgents_no-1));   %%% chebychev map au lieu de rand
 %% Orbital Period (T) 
T=abs(randn(1,SearchAgents_no)); %% Eq.(5)
Positions=initialization_CHAOS1(SearchAgents_no,dim,ub,lb); % Initialize the positions of planets
...
while t<Tmax %% Termination condition  
 ...
 M=M0*(exp(-lambda*(t/Tmax))); %% Eq. (12)   la constante gravitationnelle universelle
 ...
 cc=abs(chaos1(1,rand,SearchAgents_no-1));
 
 for i=1:SearchAgents_no
    a1(i)=cc(i)*(T(i)^2*(M*(MS(i)+m(i))/(4*pi*pi)))^(1/3); %% Eq.(23)
 end
 ...
 for i=1:SearchAgents_no
    ...
    if rand<rand
        ...
        % A flag to opposite or leave the search direction of the current planet
         ...
         U=rd>abs(chaos1(1,rand,dim-1));  %%utilisation de la carte chebychev
         if Rnorm(i)<0.5 %% Eq.(13)
            M=(rand.*(1-r)+r); %% Eq.(16)
            l=L*M*U; %% Eq.(14)
            Mv=(rand*(1-rd)+rd); %% Eq.(20)
            l1=L.*Mv.*(1-U);%% Eq.(19)
            V(i,:)=l.*(2*rand*Positions(i,:)-Positions(a,:))+l1.*(Positions(b,:)-Positions(a,:))+(1-Rnorm(i))*f*U1.*abs(chaos1(1,rand,dim-1)).*(ub-lb); %% Eq.(13a)
         else
            U2=rand>rand; %% Eq. (22) 
            %utilisatin de la carte chebychev
            V(i,:)=rand.*L.*(Positions(a,:)-Positions(i,:))+(1-Rnorm(i))*f*U2*abs(chaos1(1,rand,dim-1)).*(rand*ub-lb);  %% Eq.(13b)
         end %% End IF
